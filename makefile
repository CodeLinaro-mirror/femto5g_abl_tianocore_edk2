...
ifeq ($(BUILD_NATIVE_AARCH64),true)
	...
	export SEND_MILESTONE_CALL_LE := false
	export VERIFIED_BOOT_LE_ARB := false
	export VB_LE_DTBO_AUTH := false
	export ENABLE_LE_VARIANT := false
	export USE_RKP_ALIGNED_UDS_DERIVATION := false
...
ifeq ($(USE_RKP_ALIGNED_UDS_DERIVATION), true)
	USE_RKP_ALIGNED_UDS_DERIVATION := 1
else
	USE_RKP_ALIGNED_UDS_DERIVATION := 0
endif
...
ifneq ($(NUM_CORES_DEVICE),)
	NUM_CORES_TARGET :=$(NUM_CORES_DEVICE)
endif
ifneq ($(NUM_SILVER_CORES_DEVICE),)
    NUM_SILVER_CORES_TARGET :=$(NUM_SILVER_CORES_DEVICE)
endif
...
ABL_FV_IMG: $(EDK_TOOLS_PATH_MARK_FILE)
	...
	-D VERITY_USE_DM_MOD_CREATE=$(VERITY_USE_DM_MOD_CREATE) \
	...
	-D USE_RKP_ALIGNED_UDS_DERIVATION=$(USE_RKP_ALIGNED_UDS_DERIVATION) \
	-D WEAR_OS=$(WEAR_OS) \
	-D IOT_LOW_DDR_TARGET=$(IOT_LOW_DDR_TARGET) \
	...
	-D NUM_CORES_TARGET=$(NUM_CORES_TARGET) \
	-D NUM_SILVER_CORES_TARGET=$(NUM_SILVER_CORES_TARGET) \
	...
	-D BRANCH_PROTECTION_ENABLED=$(BRANCH_PROTECTION_ENABLED) \
	-D CFI_ENABLED=$(CFI_ENABLED) \
	-D SKIP_ORANGE_WAIT=$(SKIP_ORANGE_WAIT) \
	-D CHECK_CPU_FREQ_MITIGATION=$(CHECK_CPU_FREQ_MITIGATION) \
	-D LV_GVM_KM_VIRT=$(LV_GVM_KM_VIRT) \
	-D ENABLE_CHECK_MTE=$(ENABLE_CHECK_MTE) \
	-D EXCLUSIVE_SME_MTE=$(EXCLUSIVE_SME_MTE) \
