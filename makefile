...
ifeq ($(BUILD_NATIVE_AARCH64),true)
	...
	export ENABLE_FSP_WRAP_KEY_FEATURE := false
	export VERITY_LE := false
	export VERITY_LE_ROOTHASH_SIGNED := false
	...
	export ENABLE_SLOT_SWITCH_IMAGE_CORRUPTION := false
	export ENABLE_BOOT_DEVICE_BASED_DT_SELECTION := false
	export ENABLE_BOOT_REASON_BOOTPARAM := false
	export ENABLE_PARTIAL_AB_WITHOUT_RECOVERYINFO := false
endif
...
ifeq ($(ENABLE_FSP_WRAP_KEY_FEATURE), true)
	ENABLE_FSP_WRAP_KEY_FEATURE := 1
else
	ENABLE_FSP_WRAP_KEY_FEATURE := 0
endif
...
ifeq ($(ENABLE_SLOT_SWITCH_IMAGE_CORRUPTION), true)
        ENABLE_SLOT_SWITCH_IMAGE_CORRUPTION  := 1
else
        ENABLE_SLOT_SWITCH_IMAGE_CORRUPTION  := 0
endif

ifeq ($(ENABLE_BOOT_DEVICE_BASED_DT_SELECTION), true)
        ENABLE_BOOT_DEVICE_BASED_DT_SELECTION  := 1
else
        ENABLE_BOOT_DEVICE_BASED_DT_SELECTION  := 0
endif

ifeq ($(ENABLE_BOOT_REASON_BOOTPARAM), true)
        ENABLE_BOOT_REASON_BOOTPARAM := 1
else
        ENABLE_BOOT_REASON_BOOTPARAM := 0
endif

ifeq ($(ENABLE_PARTIAL_AB_WITHOUT_RECOVERYINFO), true)
	ENABLE_PARTIAL_AB_WITHOUT_RECOVERYINFO := 1
else
	ENABLE_PARTIAL_AB_WITHOUT_RECOVERYINFO := 0
endif
...
ifeq ($(TOOLCHAIN_GCC_MAJOR),11)
       export GCC_SIZE_OPT := -Os
else
       export GCC_SIZE_OPT := -Oz
endif
...
ABL_FV_IMG: $(EDK_TOOLS_PATH_MARK_FILE)
	...
	-D BOOT_IMG_DECRYPT=$(BOOT_IMG_DECRYPT) \
	...
	-D VERITY_LE_ROOTHASH_SIGNED=$(VERITY_LE_ROOTHASH_SIGNED) \
	...
	-D ENABLE_FSP_WRAP_KEY_FEATURE=$(ENABLE_FSP_WRAP_KEY_FEATURE) \
	...
	-D ENABLE_SLOT_SWITCH_IMAGE_CORRUPTION=$(ENABLE_SLOT_SWITCH_IMAGE_CORRUPTION) \
	-D ENABLE_BOOT_DEVICE_BASED_DT_SELECTION=$(ENABLE_BOOT_DEVICE_BASED_DT_SELECTION) \
	-D ENABLE_BOOT_REASON_BOOTPARAM=$(ENABLE_BOOT_REASON_BOOTPARAM) \
	-D ENABLE_PARTIAL_AB_WITHOUT_RECOVERYINFO=$(ENABLE_PARTIAL_AB_WITHOUT_RECOVERYINFO) \
